...
const Order = () => {
  ...
  // Component tối ưu cho từng sản phẩm trong đơn hàng
  const OrderProductItem = ({ item, onReview }) => (
    <div className="flex items-center gap-3 p-2 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
      ...
      <div className="flex-grow min-w-0">
        <h4 className="font-medium text-gray-800 text-sm truncate" title={item.product_id?.name}>{item.product_id?.name}</h4>
        <div className="flex items-center gap-2 mt-1">
          <span className="text-xs text-gray-500">x{item.qty}</span>
          <span className="text-red-500 font-semibold text-sm">{(item.cur_price * item.qty).toLocaleString()}đ</span>
        </div>
      </div>
      <button
        onClick={() => onReview(item.product_id)}
        className="px-3 py-1 text-xs rounded-md text-white font-medium transition-colors bg-[#fcd34d] hover:bg-[#fbbf24] cursor-pointer flex-shrink-0"
      >
        Đánh giá
      </button>
    </div>
  ...
  return (
    <div className="bg-white rounded-xl p-4 shadow border border-gray-100 min-h-[60vh]">
      <h2 className="text-xl font-bold text-gray-800 mb-4 text-center">Lịch sử đơn hàng</h2>
      <div className="space-y-4">
        {orders.map((order) => (
          <div key={order._id} className="bg-white rounded-xl p-4 shadow-lg border border-gray-200 hover:border-[#06AEF4] transition-all">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-3 gap-2">
              <div className="flex-grow">
                <div className="flex items-center gap-2 mb-1 flex-wrap">
                  <span className="font-bold text-blue-600">#{order._id.slice(-6).toUpperCase()}</span>
                  <span className="text-gray-500 text-xs">{new Date(order.create_at).toLocaleDateString()}</span>
                  <span className={`px-2 py-1 rounded-full text-xs font-semibold ${order.order_status === 'Đã giao' ? 'bg-green-100 text-green-700' : order.order_status === 'Đang xử lý' ? 'bg-yellow-100 text-yellow-700' : 'bg-gray-100 text-gray-600'}`}>{order.order_status}</span>
                </div>
                <p className="text-gray-600 text-sm truncate">Địa chỉ: {order.address}</p>
              </div>
              <div className="flex gap-2 flex-shrink-0">
                <button 
                  onClick={() => navigate(`/order/${order._id}`)}
                  className="px-3 py-1 border border-blue-600 bg-white hover:bg-blue-50 text-blue-600 font-medium rounded-lg text-sm transition-colors"
                >
                  Chi tiết
                </button>
                <button className="px-3 py-1 border border-[#06AEF4] bg-[#06AEF4] hover:bg-[#70d9ff] text-white font-medium rounded-lg text-sm transition-colors">
                  Hỗ trợ
                </button>
              </div>
            </div>
            ...
            <div className="space-y-2 mb-3">
              {order.items?.length > 0 ? order.items.map((item) => (
                ...
                <div className="text-gray-400 italic text-sm">Không có sản phẩm nào trong đơn hàng này.</div>
              )}
            </div>
            ...
            <div className="bg-gray-50 rounded-lg p-3">
              <div className="flex justify-between items-center text-sm">
                ...
                <div className="text-center flex-1">
                  <p className="text-gray-600 text-xs">Đã thanh toán</p>
                  <p className="font-semibold text-green-600">
                    {(order?.originalTotal ?? 0).toLocaleString()}đ
                  </p>
                </div>
                <div className="text-center flex-1">
                  <p className="text-gray-600 text-xs">Cần đổi trả</p>
